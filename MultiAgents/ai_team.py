...
if not tz_text:
    ...
    exit()
check_tz_prompt = """
Ты — строгий, но конструктивный тимлид и архитектор встроенных систем с 10-летним стажем разработки под Arduino.
Твоя задача — провести технический аудит ТЗ на разработку программного обеспечения для Arduino, которое составил ПМ (или архитектор).

Сфокусируйся на проверке программной архитектуры Arduino:
1. Корректность конечного автомата: Четко ли описаны состояния, условия переходов и триггеры (нажатия кнопки)?
2. Тайминги и неблокирующий код: Учтена ли специфика Arduino? Нет ли в логике или псевдокоде глухих задержек `delay()`, которые сломают секундомер или опрос кнопок?
3. Аппаратные особенности в коде: Заложена ли в требования обработка дребезга контактов (debounce) для кнопки?
4. Архитектура и чистота: Логичны ли требования к код-стайлу, структуре кода и выбору библиотек (например, для LCD и прерываний/таймеров)?

Инструкция по формированию ответа:

Если ТЗ безупречно по ВСЕМ пунктам, технически выполнимо, содержит идеальный псевдокод, не имеет логических дыр и готово к передаче джуну/мидлу без единого уточнения — напиши строго одно слово: "ИДЕАЛЬНО".

Если есть хотя бы одна ошибка, неточность или упущение, составь подробный список замечаний:
1. Критические баги архитектуры (то, из-за чего код не будет работать корректно — например, delay в секундомере или отсутствие антидребезга).
2. Замечания по оформлению и структуре (неполный псевдокод, размытый код-стайл, плохой выбор библиотек).

Для каждого замечания обязательно укажи:
- Цитату, заголовок или контекст из ТЗ, к которому есть претензия.
- В чем именно заключается проблема (почему такой подход создаст баги в коде).
- Конкретное предложение по исправлению (как переписать этот пункт ТЗ или псевдокод).
...
while attempt <= MAX_REVIEW_ATTEMPTS:
    ...
    feedback = ask_ollama(BOSS_MODEL, check_tz_prompt, task_type="review")

    if feedback and "ИДЕАЛЬНО" in feedback.upper():
        print("🎉 Тимлид утвердил ТЗ! Проверка пройдена успешно.")
        code_is_perfect = True
        break
    else:
        print(f"⚠️ Тимлид отклонил ТЗ и отправил на доработку.")
